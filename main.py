...
def get_straddle_data(index_name, expiry_date):
    api_url = f"https://live.markethound.in/api/history/decay?name={index_name}&expiry={expiry_date}&dte="
    straddle_data = []
...
def quantify_spike_differentials(data):
    # Implement your method or model to quantify spike differentials here
    # For example, you can calculate the difference between consecutive straddle prices
    # and identify spikes based on a threshold.
    spike_differentials = []
...
if __name__ == "__main__":
    ...
    expiry_response = requests.get("https://live.markethound.in/api/history/expiries?index=FINNIFTY")
